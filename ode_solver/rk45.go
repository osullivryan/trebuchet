...
type Simulation struct {
	...
	Environments types.SimulationEnvironment
}

func (sim *Simulation) Solve(from, h, to, max_h float64, initial_values []float64) [][]float64 {
	return RK45(
		from,
		h,
		to,
		max_h,
		initial_values,
		sim.Functions.Evaluation_function,
		sim.Functions.Integration_stop_function,
		sim.Environments)
}
...
func RK45(from, h, to, max_h float64,
	y []float64,
	fn func(float64, []float64, types.SimulationEnvironment) []float64,
	stop_fn func([]float64, types.SimulationEnvironment) bool,
	simulation_environment types.SimulationEnvironment) [][]float64 {
	eps := 0.000001
	...
	for t < to {
		...
		// generate k1
		for value := 0; value < parameters; value++ {
			k1[value] = h * fn(t1, yn, simulation_environment)[value]
		}
		...
		// generate k2
		for value := 0; value < parameters; value++ {
			k2[value] = h * fn(t2, k2p, simulation_environment)[value]
		}
		...
		// generate k3
		for value := 0; value < parameters; value++ {
			k3[value] = h * fn(t3, k3p, simulation_environment)[value]
		}
		...
		// generate k4
		for value := 0; value < parameters; value++ {
			k4[value] = h * fn(t4, k4p, simulation_environment)[value]
		}
		...
		// generate k5
		for value := 0; value < parameters; value++ {
			k5[value] = h * fn(t5, k5p, simulation_environment)[value]
		}
		...
		// generate k6
		for value := 0; value < parameters; value++ {
			k6[value] = h * fn(t6, k6p, simulation_environment)[value]
		}
